...
function AutoPlay() {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: "url('https://i.ibb.co.com/JF8yQYMh/aaaa.png')",
        minHeight: "400px"
      }}
    >
      <div className="absolute inset-0 bg-opacity-60 rounded" />
        ...
        <Slider {...settings}>
          {slideData.map((app) => (
            <div key={app.id} className="px-2 ">
              <div className="bg-blue-950 rounded-md overflow-hidden shadow text-white flex flex-col items-center justify-center p-4 h-[320px]">
                <img
                  src={app.thumbnail}
                  alt={app.name}
                  className="h-40 rounded object-contain mb-3"
                />
                <h3 className="text-lg font-bold text-green-200 mb-1">{app.title}</h3>
                <p className="text-sm text-gray-200 text-center mb-1">{app.desc}</p>
                <h4 className="text-base font-medium text-blue-300">{app.name}</h4>
              </div>
            </div>
          ))}
        </Slider>
